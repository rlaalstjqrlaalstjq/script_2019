...
class MyTk:
    def __init__(self):
        ...
        self.textbox.place(x=20, y=20, width=200)
        ...
        self.searchButton.place(x=230, y=20, width=50, height=20)
    ...
    def sendButtonAction(self):
        ...
        listargs = self.mylists[self.selectNum]
    ...
    def selection(self, event):
        listargs = self.mylists[event.widget.curselection()[0]]
    ...
    def getList(self):
        ...
        for a in root.findall('row'):
            ...
            if len(self.mylists) <= self.yIdx:
                self.mylists.append([])

            self.mylists[self.yIdx].append(a.findtext('GET_NAME'))
            self.mylists[self.yIdx].append(a.findtext('REG_DATE'))
            self.mylists[self.yIdx].append(a.findtext('GET_GOOD'))
            self.mylists[self.yIdx].append(a.findtext('GET_THING').replace("<br>", "\n"))

            self.mylist.insert(self.yIdx, self.mylists[self.yIdx][TextType.LostItem.value])
